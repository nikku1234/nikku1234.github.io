...
const stack = ["C++", "Kotlin", "TypeScript", "Java", "Python", "AWS", "Docker", "PyTorch"];
const pipeline = [
...
const consoleLines = [["$", "deploy arqa-simulation --fleet=large"], ["✓", "scenario graph compiled"], ["✓", "workers provisioned across region"], ["→", "streaming performance telemetry"], ["●", "quality gate: READY"]];
...
export default function Home() {
  ...
  return <div className="home-page">
    <section className="hero-section" ref={heroRef} onPointerMove={moveHero}>
      ...
      <div className="hero-copy"><p className="eyebrow"><span /> Software engineer · robotics systems</p><h1>I build systems for machines that <em>move.</em></h1><p className="hero-lede">I’m Nikhil, an SDE II at Amazon Robotics. I build simulation and performance infrastructure that helps large-scale robotic systems ship with confidence.</p><div className="hero-actions"><Link className="button button-primary magnetic-button" to="/experience">Explore my work <span>↗</span></Link><a className="text-link" href="mailto:nikhilramesh96@gmail.com">Start a conversation <span>→</span></a></div><div className="hero-meta"><span>ARQA AUTOMATION</span><span>PERFORMANCE PLATFORM</span><span>AMAZON ROBOTICS</span></div></div>
    ...
    <section className="workbench-section"><div className="workbench-heading"><p className="eyebrow">Inside the work</p><h2>From simulated world<br />to real confidence.</h2><p>My work lives at the boundary between software and physical systems: automated robot quality assurance, performance platforms, and the distributed infrastructure underneath them.</p></div><div className="system-console" aria-label="Animated example deployment console"><div className="console-bar"><span><i /><i /><i /></span><b>arqa / performance-platform</b><span>LIVE</span></div><div className="console-body">{consoleLines.map(([symbol, text], index) => <p key={text} className={index < lineCount ? "is-visible" : ""}><span>{symbol}</span>{text}{index === lineCount - 1 && <i />}</p>)}</div><div className="console-stats"><div><span>WORKLOAD</span><b>DISTRIBUTED</b></div><div><span>MODE</span><b>SIMULATION</b></div><div><span>STATE</span><b className="status-ready">NOMINAL</b></div></div></div></section>
